...
const consistentHash = (kid, nids) => {
  ...
  let nextNode = nids[0];
  let minDiff = BigInt(-1);

  for (const nid of nids) {
    let diff = idToNum(nid) - kidNum;
    if (diff > BigInt(0)) {
      if (minDiff === BigInt(-1) || diff < minDiff) {
        minDiff = diff;
        nextNode = nid;
      }
    }
...
const rendezvousHash = (kid, nids) => {
  let ans = nids[0];
  let pivot = idToNum(getID(kid + nids[0]));

  for (const nid of nids) {
    const tmp = idToNum(getID(kid + nid));
    if (tmp > pivot) {
      pivot = tmp;
      ans = nid;
    }
